    ...
    <Layout title="Home" description="V3 External API Documentation">
      <div className={styles.hero}>
        ...
        <div className={styles.container}>
          ...
          <h1 className={styles.title}>
            Build faster with<br/>our External API
          </h1>
          
          <p className={styles.subtitle}>
            Programmatically manage Domains, Cautions, Disclaimers, and Departments with our robust, secure, and high-performance REST API.
          </p>
      ...
      <div className={styles.endpoints}>
        <div className={styles.container}>
          ...
          <p className={styles.sectionSubtitle}>Complete REST API for email infrastructure management</p>
          
          <div className={styles.endpointGrid}>
            {[
              { name: 'Health', desc: 'System monitoring', path: '/docs/api/health' },
              { name: 'Domains', desc: 'Domain management', path: '/docs/api/domains' },
              { name: 'Cautions', desc: 'Email cautions', path: '/docs/api/cautions' },
              { name: 'Disclaimers', desc: 'Email disclaimers', path: '/docs/api/disclaimers' },
              { name: 'Departments', desc: 'Organization structure', path: '/docs/api/departments' },
              { name: 'Self Checks', desc: 'API verification', path: '/docs/api/self-checks' },
            ].map((endpoint, idx) => (
              <a key={idx} href={`${endpoint.path}`} className={styles.endpointCard}>
                <div>
                  <h3>{endpoint.name}</h3>
                  <p>{endpoint.desc}</p>
                </div>
